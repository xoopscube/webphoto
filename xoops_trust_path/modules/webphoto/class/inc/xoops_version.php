...
class webphoto_inc_xoops_version extends webphoto_inc_base_ini {
	 ...
	 * @icon XCL SVG
	 */
	public function _build_basic() {
		$module_icon = 'module_icon.php';
		//if ( file_exists( $this->_MODULE_DIR . '/images/module_icon.png' ) ) {
		//$module_icon = 'images/module_icon.png';
		if ( file_exists( $this->_MODULE_DIR . '/images/module_webphoto.svg' ) ) {
			$module_icon = 'images/module_webphoto.svg';
		}

		$arr = array();

		$arr['name'] = $this->_DIRNAME;
		//$arr['name']        = $this->_constant( 'NAME' ) . '('.$this->_DIRNAME.')';
		$arr['description'] = $this->_constant( 'DESC' );
		$arr['author']      = "Kenichi Ohwada. Gigamaster version XCL PHP7";
		$arr['credits']     = "Kenichi Ohwada, GIJOE (Peak.ne.jp), Daniel Branco (Bluetopia.pt), Nuno Luciano (Gigamaster version XCL)\n";
		$arr['help']        = "";
...
// Comments
	...
	public function _build_comments() {
		$arr = array();
...
// Notification
	...
	public function _build_notification() {
		$arr = array();
...
// Sub Menu
	...
	public function _build_sub_array_const( $name, $url ) {
		return $this->_build_sub_array( $this->_constant( $name ), $url );
	}

	public function _build_sub_array( $name, $url ) {
		$arr = array(
			'name' => $name,
			'url'  => $url,
		);
	...
	public function _build_sub_url_fct( $fct ) {
	...
	public function _build_sub_url_op( $op ) {
		if ( $this->_cfg_use_pathinfo ) {
			$str = 'index.php/' . $op . '/';
		} else {
			$str = 'index.php?op=' . $op;
	...
	public function _build_sub_url_category( $id ) {
		if ( $this->_cfg_use_pathinfo ) {
...
// keep block's options
	...
	public function _build_blocks_top_options() {
		$str = $this->_DIRNAME;
	...
	public function _build_blocks_top_p_options() {
		$str = $this->_DIRNAME;
...
// langauge
	...
	public function _constant_name( $name ) {
		return strtoupper( '_MI_' . $this->_DIRNAME . '_' . $name );
	}


// blocks handler
	public function _blocks_keep_option( $blocks ) {
// Keep Block option values when update (by nobunobu) for XOOPS 2.0.x
	...
	public function _get_newblocks_rows( $func_num, $limit = 0, $offset = 0 ) {
		$sql = "SELECT * FROM " . $this->_db->prefix( 'newblocks' );
		$sql .= " WHERE mid=" . (int) $this->_MODULE_ID;
	...
	public function _delete_newblocks( $bid ) {
	...
	public function _get_newblocks_row( $func_num, $show_func, $func_file ) {
		$sql = "SELECT * FROM " . $this->_db->prefix( 'newblocks' );
		$sql .= " WHERE mid=" . (int) $this->_MODULE_ID;
		$sql .= " AND func_num=" . (int) $func_num;
	...
	public function _blocks_msg( $local_msgs ) {
...
// cat table
	public function _get_top_cat_rows() {
...
// group_permission
	public function _init_group_permission( $dirname, $trust_dirname ) {
...
// xoops param
	public function _init_is_module_admin() {
		global $xoopsUser;
...
// workdir
	public function _init_workdir( $dirname, $trust_dirname ) {
...
// ini
	public function _init_ini( $dirname, $trust_dirname ) {
		$this->init_base_ini( $dirname, $trust_dirname );
