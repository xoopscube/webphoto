 ...
 * @brief class webphoto_edit_mail_check
...
class webphoto_edit_mail_check {
	...
	public $_DENY_MAIL_FROM_ARRAY = array(
		...
		'mine.nu',
		'51444.tv',
		'nyan_nyan_cat_2004@yahoo.co.jp',
		'motto.zapto.org',
		'i-towns.net',
		'ori-g.net',
		'jewelry.polty.cc',
...
// parse_mail
	...
	public function check_mail( $param ) {
		...
		[ $subject, $rotate ] = $this->parse_subject_rotate( $subject );
...
// ok if us_ascii
	...
	public function check_mail_from( $mail_from ) {
		...
		foreach ( $this->_DENY_MAIL_FROM_ARRAY as $iValue ) {
// Function ereg() is deprecated
			$pattern = '/' . $iValue . '/i';
...
// proofread body
	...
	public function replace_tel( $text ) {
// Function ereg() is deprecated
...
// attach
	...
	public function check_attach_ext( $ext ) {
		return $ext && in_array( $ext, $this->_allowed_exts );
	}

	public function check_attach_mimetype( $type ) {
		return $type && in_array( $type, $this->_allowed_mimes );
	}

	public function check_attach_maxbyte( $content ) {
		return ! ( strlen( $content ) > $this->_ATTACH_MAXBYTE );
	}


// utility

	public function check_mail_addr( $addr ) {
